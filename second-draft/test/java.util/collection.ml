...
let i_collection_param = "COLLECTION_PARAM"
let i_collection =
...
let () =
  let ctx =
    ...
    let vm5 = varmap_addvar vm4
                            ...
                            (Bot, Bot)
    in
    context_init cc sc vm5
  in
  let () = typecheck (interface_ok ctx i_collection) in
  let () = Format.printf "%s\n" (Pretty_print.pinter_t ctx i_collection) in
  ()
