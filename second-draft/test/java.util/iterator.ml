...
let i_iterator_param = "ITERATOR_PARAM"
let i_iterator =
  let rConsumer = (* Consumer<? super ITERATOR_PARAM> *)
    Instance("Consumer", varmap_addvar empty_varmap
                                       Consumer.i_consumer_param
                                       (TVar i_iterator_param, Top)
            )
  in
  Interface ( "Iterator"
            , [i_iterator_param]
...
let () =
  let ctx =
    ...
    let vm = varmap_addvar (varmap_addvar empty_varmap
                                          Consumer.i_consumer_param
                                          (TVar i_iterator_param, Top))
                           i_iterator_param
                           (Bot, Bot)
  ...
  in
  let () = typecheck (interface_ok ctx i_iterator) in
  let () = Format.printf "%s\n" (Pretty_print.pinter_t ctx i_iterator) in
  ()
