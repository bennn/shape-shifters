...
let i_spliterator_param = "SPLITERATOR_PARAM"
let i_spliterator =
...
let () =
  let ctx =
    ...
    let vm1 = varmap_addvar empty_varmap
                           ...
                           (TVar i_spliterator_param, Top)
    in
    let vm2 = varmap_addvar vm1 i_spliterator_param (Bot, Bot) in
    context_init cc sc vm2
  in
  let () = typecheck (interface_ok ctx i_spliterator) in
  let () = Format.printf "%s\n" (Pretty_print.pinter_t ctx i_spliterator) in
  ()
