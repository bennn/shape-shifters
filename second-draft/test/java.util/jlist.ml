...
let i_list_param = "LIST_PARAM"
let i_list =
  Interface ( "List"
            , [i_list_param]
            , [(NoCond, Collection.i_collection)]
            , []
...
let () =
  let ctx =
    let cc =
      ...
      (StringMap.add (name_of_inter_t Iterable.i_iterable) (I Iterable.i_iterable)
    ...
    let vm4 = varmap_addvar vm3
                            Iterable.i_iterable_param
                            (Bot, TVar i_list_param)
    in
    let vm5 = varmap_addvar vm4
                            Collection.i_collection_param
                            (Bot, TVar i_list_param)
    in
    let vm6 = varmap_addvar vm5
                            i_list_param
                            (Bot, Bot)
    in
    context_init cc sc vm6
  in
  let () = typecheck (interface_ok ctx i_list) in
  let () = Format.printf "%s\n" (Pretty_print.pinter_t ctx i_list) in
  ()
